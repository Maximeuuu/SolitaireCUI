...
public class Texte2D
{
	...
	*/
	...
	{ //assemble plusieurs Texte2D avec un écart de taille "espacement"
		if( tabElement2D.length == 0 ){ return null;} //s'il n'y a pas d'éléments à ajouter
		...
		for( int cptElt=1; cptElt<tabElement2D.length; cptElt++ )
		{
			...
			{
				for( int cptCol=0; cptCol<tmp.largeur; cptCol++ )
				{
					tmp.setCase(cptCol,cptLigTmp," "); // ajout des espaces lignes par lignes, colonnes par colonnes
				}
				cptLigTmp++;
			}
...
/*POSSIBLEMENT INUTILE ET REMPLACES PAR "COMPLETER()"
...
*/
	...
	/*COMPARAISONS*/
	private static int maxHauteur( Texte2D[] tabElement2D )
